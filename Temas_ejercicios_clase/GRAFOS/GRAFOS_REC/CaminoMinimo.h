...
class CaminoMinimo {
	protected:
		...
		int* D; 				// vector de distancias minimas
	...
	public:
		...
		void Dijkstra(GrafoMatriz g, int origen); 	// calcular lo caminos minimos del vertice origen al resto de los vertices
...
int CaminoMinimo::minimo(bool F[]){					//metodo privado para encontrar el indice del vertice del grafo cuya distancia al origen sea m�nima (elegido entre los no marcados).
...
void CaminoMinimo::Dijkstra(GrafoMatriz g, int origen) {
	bool * F;										//vector booleano que almacena los vertices marcados
	...
	// Pasos para marcar los n-1 vertices. Algoritmo voraz
...
void CaminoMinimo::recuperaCamino(int v) { 			//metodo para regresar a sucesivos (anteriores) vertices que conforman el camino m�nimo
