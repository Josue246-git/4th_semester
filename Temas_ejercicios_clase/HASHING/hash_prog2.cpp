...
class TablaDispersa{
	...
	private:
		long transformaCadena(char * c); // calcular la posici�n de un elemento en la tabla
};
...
int TablaDispersa::direccion(char *clave){
	...
	p = d % final;  	// aplica aritm�tica modular para obtener direcci�n base
	
	while (tabla[p]!= NULL && strcmp(tabla[p]->elCodigo(),clave)!= 0){   // bucle de exploraci�n
		i++;
		p = p + i * i;	//exploraci�n cuadr�tica
...
void TablaDispersa::insertar(CasaRural r){
	...
	if (factorCarga > 0.5){
		cout << endl  << "!! Factor de carga supera el 50 % !!.... Conviene aumentar el tama�o." << endl;
	}
}
